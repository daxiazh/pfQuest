...
-- @return 任务节点对象，包含 type, title, rewards, children 字段
local function FindRewards(questId, visited, depth)
    if not questId then
        return nil
    end
    depth = depth or 0
    ...
    if rewards then
        ...
        if table.getn(validRewards) > 0 then
            questNode.rewards = validRewards
        end
    end
            ...
            if questData["pre"] then
                for _, prequest in pairs(questData["pre"]) do
                    if prequest == questId then
                        local childNode = FindRewards(qid, visited, depth + 1)
                        if childNode then
                            children[qid] = childNode
                        end
                    end
                end
            end
...
local function str2rgb(text)
    ...
    for i = 1, l, 3 do
        counter = compat.mod(counter * 8161, 4294967279) + (string.byte(text, i) * 16776193) +
    ...
    if meta["quest"] then
        ...
        for qid = 1, GetNumQuestLogEntries() do
            ...
            if meta["quest"] == qtitle then
                ...
                local symbol = (complete or objectives == 0) and "|cff555555[|cffffcc00?|cff555555]|r " or
                                   "|cff555555[|cffffcc00!|cff555555]|r "
                ...
                if objectives then
                    for i = 1, objectives, 1 do
                        ...
                        if type == "monster" then
                            -- kill
                            local i, j, monsterName, objNum, objNeeded = strfind(text, pfUI.api
                                .SanitizePattern(QUEST_MONSTERS_KILLED))
                            if monsterName and meta["spawn"] == monsterName then
                                catch_obj = true
                                local r, g, b = pfMap.tooltip:GetColor(objNum, objNeeded)
                                tooltip:AddLine("|cffaaaaaa- |r" .. monsterName .. ": " .. objNum .. "/" .. objNeeded,
                                    r, g, b)
                            end
                        elseif table.getn(meta["item"]) > 0 and type == "item" and meta["droprate"] then
                            -- loot
                            local i, j, itemName, objNum, objNeeded = strfind(text, pfUI.api
                                .SanitizePattern(QUEST_OBJECTS_FOUND))
                            ...
                            for mid, item in pairs(meta["item"]) do
                                if item == itemName then
                                    ...
                                    tooltip:AddLine(
                                        "|cffaaaaaa- |r" .. itemName .. ": " .. objNum .. "/" .. objNeeded ..
                                            " |cff555555[|cff" .. lootcolor .. meta["droprate"] .. "%|cff555555]", r, g,
                                        b)
                                end
                            end
                        elseif table.getn(meta["item"]) > 0 and type == "item" and meta["sellcount"] then
                            ...
                            for mid, item in pairs(meta["item"]) do
                                if item == itemName then
                                    catch_obj = true
                                    local r, g, b = pfMap.tooltip:GetColor(objNum, objNeeded)
                                    local sellcount = tonumber(meta["sellcount"]) > 0 and " |cff555555[|cffcccccc" ..
                                                          meta["sellcount"] .. "x" .. "|cff555555]" or ""
                                    tooltip:AddLine(
                                        "|cffaaaaaa- |r" .. pfQuest_Loc["Buy"] .. ": " .. itemName .. ": " .. objNum ..
                                            "/" .. objNeeded .. sellcount, r, g, b)
                                end
                            end
                        end
                    end
        ...
        if not catch_obj then
            ...
            if meta["item"] and meta["item"][1] and meta["droprate"] then
                for mid, item in pairs(meta["item"]) do
                    ...
                    local dr, dg, db = pfMap.tooltip:GetColor(tonumber(meta["droprate"]), 100)
                    local lootcolor = string.format("%02x%02x%02x", dr * 255, dg * 255, db * 255)
                    tooltip:AddLine("|cffaaaaaa- |r" .. item .. " |cff555555[|cff" .. lootcolor .. meta["droprate"] ..
                                        "%|cff555555]", .7, .7, .7)
                end
            end
            ...
            if meta["item"] and meta["item"][1] and meta["sellcount"] then
                for mid, item in pairs(meta["item"]) do
                    catchFallback = true
                    local sellcount =
                        tonumber(meta["sellcount"]) > 0 and " |cff555555[|cffcccccc" .. meta["sellcount"] .. "x" ..
                            "|cff555555]" or ""
            ...
            if not catchFallback and meta["spawn"] and not meta["texture"] then
                catchFallback = true
                tooltip:AddLine("|cffaaaaaa- |r" ..
                                    (meta["spawntype"] and meta["spawntype"] == "Trigger" and pfQuest_Loc["Explore"] or
                                        meta["spawn"]), .7, .7, .7)
            end
            ...
            if not catchFallback and meta["texture"] and meta["qlvl"] then
                ...
                local qlvlstr =
                    pfQuest_Loc["Level"] .. ": " .. pfMap:HexDifficultyColor(meta["qlvl"]) .. meta["qlvl"] .. "|r"
                local qminstr = meta["qmin"] and " / " .. pfQuest_Loc["Required"] .. ": " ..
                                    pfMap:HexDifficultyColor(meta["qmin"], true) .. meta["qmin"] .. "|r" or ""
        ...
        if questNode then
            ...
            if questNode.rewards then
                ...
                for _, reward in ipairs(questNode.rewards) do
                    ...
                    tooltip:AddLine("    " .. itemColor .. itemName .. "|r", .7, .7, .7)
                end
            end

            -- 再显示后续任务链的奖励
            if questNode.children then
                tooltip:AddLine("|cff00ff00 --- 任务链奖励 ---")

                -- 递归显示道具奖励（不包括根节点任务，只显示子任务链）
                local function DisplayRewards(questNode, depth)
                    depth = depth or 0
                    local indent = string.rep("  ", depth) -- 根据深度添加缩进

                    if questNode.type == "quest" then
                        -- 显示任务标题（如果有奖励或子任务）
                        if questNode.rewards or questNode.children then
                            tooltip:AddLine(indent .. "|cff555555[|cffffcc00!|cff555555]|r " .. questNode.title, 1, 1, .7)
                            ...
                            if questNode.rewards then
                                for _, reward in ipairs(questNode.rewards) do
                                    ...
                                    local itemQuality = reward[2]
                                    local itemName =
                                        pfDB["items"] and pfDB["items"]["loc"] and pfDB["items"]["loc"][itemId] or
                                            "未知物品"
                                    local itemColor = "|c" ..
                                                          string.format("%02x%02x%02x%02x", 255,
                                    ...
                                    tooltip:AddLine(indent .. "    " .. itemColor .. itemName .. "|r", .7, .7, .7)
                                end
                            end
                            ...
                            if questNode.children then
                                for _, childNode in pairs(questNode.children) do
                                    DisplayRewards(childNode, depth + 1)
                                end
                            end
                        end
                    end
        ...
        if meta["item"][1] and meta["itemid"] and not meta["itemlink"] then
            ...
            if itemQuality then
                local itemColor = "|c" ..
                                      string.format("%02x%02x%02x%02x", 255, ITEM_QUALITY_COLORS[itemQuality].r * 255,
                        ITEM_QUALITY_COLORS[itemQuality].g * 255, ITEM_QUALITY_COLORS[itemQuality].b * 255)
        ...
        elseif meta["item"][1] then
            ...
            tooltip:AddLine(
                "|cffffffff" .. pfQuest_Loc["Loot"] .. ": " .. item .. " |cff555555[|r" .. meta["droprate"] ..
                    "%|cff555555]", r, g, b)
        end
    end
...
function pfMap:AddNode(meta)
    ...
    local sindex = string.format("%s:%s:%s:%s:%s:%s", (addon or ""), (map or ""), (coords or ""), (title or ""),
        (layer or ""), (spawn or ""), (item or ""))
    ...
    if not meta["cluster"] and not meta["texture"] then
        ...
        if not unifiedcache[title][map][node_index] then
            ...
            unifiedcache[title][map][node_index] = {
                meta = unified_meta,
                coords = {}
            }
        end
...
function pfMap:NodeEnter()
    ...
    tooltip:SetText(this.spawn .. (pfQuest_config.showids == "1" and " |cffcccccc(" .. this.spawnid .. ")|r" or ""), .3,
        1, .8)
        ...
        if pfMap.nodes[addon][map] then
            for coords, node in pairs(pfMap.nodes[addon][map]) do
                ...
                if (pfQuest_config["routecluster"] == "1" and pfMap.pins[i].layer >= 9) or
                    ...
                    (pfQuest_config["routestarter"] == "1" and pfMap.pins[i].layer == 2) or pfMap.pins[i].arrow == true then
        ...
        if data[mapID] and minimap_sizes[mapID] and pfMap:HasMinimap(mapID) then
            for coords, node in pairs(data[mapID]) do
                ...
                if pfUI.minimap then
                    display = (abs(xPos) + 8 < pfMap.drawlayer:GetWidth() / 2 and abs(yPos) + 8 <
                                  pfMap.drawlayer:GetHeight() / 2) and true or nil
                else
    ...
    if pfMap.queue_update or transition or pfMap.highlight ~= hlstate or shiftstate ~= hidecluster then
        ...
        for frame, data in pairs(pfMap.highlightdb) do
            ...
            elseif not highlight and pfMap.highlight then
                -- fade node
                transition = frame:Animate(frame.defsize, tonumber(pfQuest_config["nodefade"]) or 0.3, fps) or
                                 transition
